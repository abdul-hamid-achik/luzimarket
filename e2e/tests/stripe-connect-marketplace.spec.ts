...
test.describe('Stripe Connect Marketplace', () => {
  ...
  test.describe('Order Tracking Integration', () => {
    test('should track orders after checkout', async ({ page }) => {
      ...
      // Wait for cart sheet or navigate to cart
      const cartSheetVisible = await page.waitForSelector('[role="dialog"], [data-testid="cart-sheet"]', { timeout: 5000 }).catch(() => null);

      if (cartSheetVisible) {
        await page.getByTestId('checkout-link').first().click();
      } else {
        // If cart sheet doesn't open, navigate to checkout directly
