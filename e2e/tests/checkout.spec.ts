...
test.describe('Checkout Flow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const addButton = firstProduct.locator('button').filter({ hasText: /Add to cart|Agregar al carrito/i }).first();
    await addButton.click();
    await page.waitForTimeout(1000); // Give more time for cart to update
  });
  ...
  test('should open cart sidebar', async ({ page }) => {
    ...
    test('should validate checkout form', async ({ page }) => {
      ...
      // Select guest checkout first - be more aggressive
      await page.waitForLoadState('networkidle');
      
      const guestLabel = page.locator('label[for="guest"]');
      if (await guestLabel.isVisible({ timeout: 3000 })) {
        await guestLabel.click({ force: true });
        await page.waitForTimeout(1000);
      }

      // Wait for checkout form to load - ensure guest form is visible
    ...
    test('should fill checkout form', async ({ page }) => {
      ...
      // Wait for form to be visible after guest selection
