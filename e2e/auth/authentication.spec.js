...
test.describe('Authentication Flow', () => {
    test.describe('Guest State', () => {
        test('should show guest state when not logged in', async ({ page }) => {
            ...
            // Check if the root React div has content
            const rootContent = await page.locator('#root').innerHTML();
            console.log('React root content:', rootContent.substring(0, 1000));

            // Try to find any navbar elements
            const navbarExists = await page.locator('.navbar, #NavbarPrincipal, .NavbarIndex').count();
            console.log('Navbar elements found:', navbarExists);

            // If navbar doesn't exist, this is a rendering issue
            if (navbarExists === 0) {
                throw new Error('Navbar is not rendering - React app may have errors');
            }

            // Check for guest indicators with more flexible selectors
