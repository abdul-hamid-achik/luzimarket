...
export async function POST() {
    try {
        ...
        const sessionId = (insertResult[0] as any).id;
        ...
        const token = jwt.sign({ sessionId, isGuest: true }, jwtSecret, { expiresIn: '7d' });
        return NextResponse.json({ token, sessionId }, { status: StatusCodes.OK });
    } catch (error) {
