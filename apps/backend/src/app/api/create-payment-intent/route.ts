...
import { validatePrefixedId, isLegacyUUID } from '@/lib/ids';
...
function isValidOrderId(str: string): boolean {
    ...
    return validatePrefixedId(str) || isLegacyUUID(str);
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Only include customer field if user has a valid Stripe customer ID
        if (user.stripe_customer_id && user.stripe_customer_id.trim() !== '') {
            paymentIntentData.customer = user.stripe_customer_id;
        }
        ...
        // Only include stripe_customer_id if user has a valid one
        if (user.stripe_customer_id && user.stripe_customer_id.trim() !== '') {
            orderUpdateData.stripe_customer_id = user.stripe_customer_id;
        }

        await dbService.update(orders, orderUpdateData, eq(orders.id, orderId));
