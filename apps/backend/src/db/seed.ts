    ...
    try {
        ...
        if (DB_MODE === 'offline') {
            ...
            try {
                ...
                console.log('✅ Usuarios realistas sembrados exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando usuarios (SQLite):', err); }
            ...
            try {
                ...
                console.log('✅ Registros de empleados sembrados exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando empleados (SQLite):', err); }
            ...
            try {
                ...
                if (vendorUsers.length > 0) {
                    ...
                    console.log('✅ Registros de proveedores sembrados exitosamente (SQLite)');
                }
            } catch (err) { console.log('❌ Error sembrando proveedores (SQLite):', err); }
            ...
            try {
                ...
                console.log('✅ Sesiones y carritos sembrados exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando sesiones y carritos (SQLite):', err); }

            // Seed favorites for customers with realistic distribution
            try {
                const customerUsers = await dbInstance.select().from(schemaInstance.users).where(eq(schemaInstance.users.role, 'customer'));
                const variants = await dbInstance.select().from(schemaInstance.productVariants);

                // Create a more realistic favorites distribution
                // Some products should be more popular than others
                const popularVariants = faker.helpers.arrayElements(variants, Math.min(10, variants.length));
                const regularVariants = variants.filter(v => !popularVariants.includes(v));

                for (const customer of customerUsers) {
                    // Each customer has 2-8 favorite items
                    const numFavorites = faker.number.int({ min: 2, max: 8 });
                    const favoriteVariants: typeof variants = [];

                    // 70% chance to pick from popular variants
                    for (let i = 0; i < numFavorites; i++) {
                        if (Math.random() < 0.7 && popularVariants.length > 0) {
                            const popularVariant = faker.helpers.arrayElement(popularVariants);
                            if (!favoriteVariants.find(fv => fv.id === popularVariant.id)) {
                                favoriteVariants.push(popularVariant);
                            }
                        } else if (regularVariants.length > 0) {
                            const regularVariant = faker.helpers.arrayElement(regularVariants);
                            if (!favoriteVariants.find(fv => fv.id === regularVariant.id)) {
                                favoriteVariants.push(regularVariant);
                            }
                        }

                        // Break if we have enough unique favorites
                        if (favoriteVariants.length >= numFavorites) break;
                    }

                    // Fill remaining slots from any available variants
                    while (favoriteVariants.length < numFavorites && favoriteVariants.length < variants.length) {
                        const randomVariant = faker.helpers.arrayElement(variants);
                        if (!favoriteVariants.find(fv => fv.id === randomVariant.id)) {
                            favoriteVariants.push(randomVariant);
                        }
                    }
                ...
                console.log('✅ Favoritos de clientes con distribución realista sembrados exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando favoritos (SQLite):', err); }
            ...
            try {
                ...
                if (productsWithVariants.length > 0 && allUsers.length > 0) {
                    ...
                    for (let i = 0; i < numberOfOrders; i++) {
                        ...
                        // Create more realistic order dates (last 6 months with higher frequency in recent weeks)
                        let orderDate;
                        if (Math.random() < 0.4) {
                            // 40% recent orders (last 30 days)
                            orderDate = new Date(Date.now() - faker.number.int({ min: 0, max: 30 * 24 * 60 * 60 * 1000 }));
                        } else if (Math.random() < 0.7) {
                            // 30% medium recent (30-90 days ago)
                            orderDate = new Date(Date.now() - faker.number.int({ min: 30 * 24 * 60 * 60 * 1000, max: 90 * 24 * 60 * 60 * 1000 }));
                        } else {
                            // 30% older orders (90-180 days ago)
                            orderDate = new Date(Date.now() - faker.number.int({ min: 90 * 24 * 60 * 60 * 1000, max: 180 * 24 * 60 * 60 * 1000 }));
                        }
                    ...
                    console.log('✅ Órdenes realistas sembradas exitosamente (SQLite)');
                } else {
                    console.log('⚠️  No se encontraron productos o usuarios para crear órdenes');
                }
            } catch (err) { console.log('❌ Error sembrando órdenes realistas (SQLite):', err); }
                ...
                for (const type of petitionTypes) {
                    for (const status of petitionStatuses) {
                        await dbInstance.insert(schemaInstance.petitions).values({
                            ...
                            status,
                            createdAt: new Date(Date.now() - faker.number.int({ min: 0, max: 90 * 24 * 60 * 60 * 1000 }))
                        }).onConflictDoNothing();
                    }
                }
            ...
            try {
                ...
                console.log('✅ Slides de página principal sembradas exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando slides de página principal (SQLite):', err); }
            ...
            try {
                ...
                for (const customer of customerUsers) {
                    ...
                    for (let i = 0; i < numAddresses; i++) {
                        await dbInstance.insert(schemaInstance.userAddresses).values({
                            ...
                            isDefault: i === 0, // Use boolean directly
                ...
                console.log('✅ Direcciones de usuarios sembradas exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando direcciones de usuarios (SQLite):', err); }
                ...
                for (const product of allProducts) {
                    ...
                    for (const customer of selectedCustomers) {
                        await dbInstance.insert(schemaInstance.productReviews).values({
                            ...
                            isApproved: Math.random() < 0.9,
                            createdAt: new Date(Date.now() - faker.number.int({ min: 0, max: 60 * 24 * 60 * 60 * 1000 })),
                        }).onConflictDoNothing();
                    }
                }
                console.log('✅ Reseñas de productos sembradas exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando reseñas de productos (SQLite):', err); }
            ...
            try {
                ...
                console.log('✅ Códigos de descuento sembrados exitosamente (SQLite)');
            } catch (err) { console.log('❌ Error sembrando códigos de descuento (SQLite):', err); }
...
async function loadSchemaAndRunSeed() {
    try {
        if (DB_MODE === 'offline') {
            ...
            await seed(sqliteDb, mainSchema);
        } else {
