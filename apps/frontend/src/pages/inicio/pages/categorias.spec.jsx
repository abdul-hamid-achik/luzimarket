...
describe('Categorias Page', () => {
    ...
    it('renders category cards with correct structure', () => {
        ...
        // Check that buttons have correct links (now SEO-friendly)
        viewProductsButtons.forEach(button => {
            expect(button.closest('a')).toHaveAttribute('href', expect.stringContaining('/categorias/'));
        });
    });
    ...
    it('renders call to action section', () => {
        ...
        // Use getAllByText for elements that appear multiple times due to demo categories
        const allProductsButtons = screen.getAllByText('Ver Todos los Productos');
    ...
    it('renders category descriptions correctly', () => {
        ...
        // Check for demo category descriptions (use getAllByText since they may appear multiple times)
        const flowerDescription = screen.getAllByText('Hermosas flores y arreglos florales para toda ocasión');
        expect(flowerDescription.length).toBeGreaterThan(0);
    ...
    it('renders icons for demo categories', () => {
        ...
        // Check for icon test IDs (use getAllByTestId since they may appear multiple times)
    ...
    it('has proper link navigation', () => {
        ...
        // Check call-to-action links (get all instances and check the first one)
        const allProductsLinks = screen.getAllByText('Ver Todos los Productos');
        const firstProductsLink = allProductsLinks[allProductsLinks.length - 1].closest('a'); // Get the last one (call-to-action section)
        const blogLink = screen.getByText('Leer Nuestro Blog').closest('a');

        expect(firstProductsLink).toHaveAttribute('href', '/handpicked/productos');
