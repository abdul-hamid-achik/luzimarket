...
function loadMessages(context) {
  ...
  // Check if files have been modified to invalidate cache
  try {
    const enStat = fs.statSync(enPath);
    const esStat = fs.statSync(esPath);
    const currentModTime = Math.max(enStat.mtimeMs, esStat.mtimeMs);

    if (cachedMessages && currentModTime <= lastModTime) {
...
const rules = {
  'messages-parity': {
    ...
    create(context) {
      // Always run the check - cache invalidation will handle performance
      cachedMessages = null; // Force fresh load
  ...
  'messages-sorted': {
    ...
    create(context) {
      // Always run the check - removed once-only limitation

      const { en, es } = loadMessages(context);
      if (!en || !es) return {};

      // More lenient sorting check - normalize JSON with consistent formatting
      const normalizeJson = (obj) => JSON.stringify(sortDeep(obj), null, 2);
      const sortedEn = sortDeep(en);
      const sortedEs = sortDeep(es);

      const enNormalized = normalizeJson(en);
      const enSortedNormalized = normalizeJson(sortedEn);
      const esNormalized = normalizeJson(es);
      const esSortedNormalized = normalizeJson(sortedEs);

      const enOk = enNormalized === enSortedNormalized;
      const esOk = esNormalized === esSortedNormalized;

      if (!enOk || !esOk) {
        const parts = [];
        if (!enOk) {
          parts.push('messages/en.json');
          // Debug: Log first few differences
          const enLines = enNormalized.split('\n');
          const enSortedLines = enSortedNormalized.split('\n');
          console.log('EN sorting issue - first difference at line:');
          for (let i = 0; i < Math.min(enLines.length, enSortedLines.length, 10); i++) {
            if (enLines[i] !== enSortedLines[i]) {
              console.log(`Line ${i + 1}:`);
              console.log(`Current: ${enLines[i]}`);
              console.log(`Expected: ${enSortedLines[i]}`);
              break;
            }
          }
        }
        if (!esOk) {
          parts.push('messages/es.json');
          // Debug: Log first few differences  
          const esLines = esNormalized.split('\n');
          const esSortedLines = esSortedNormalized.split('\n');
          console.log('ES sorting issue - first difference at line:');
          for (let i = 0; i < Math.min(esLines.length, esSortedLines.length, 10); i++) {
            if (esLines[i] !== esSortedLines[i]) {
              console.log(`Line ${i + 1}:`);
              console.log(`Current: ${esLines[i]}`);
              console.log(`Expected: ${esSortedLines[i]}`);
              break;
            }
          }
        }
    ...
    create(context) {
      ...
      if (varToBase.size) {
        for (const [v, base] of varToBase.entries()) {
          // More precise regex: ensure variable name is followed by opening parenthesis
          const re = new RegExp('\\b' + v.replace(/[$]/g, '\\$&') + "\\s*\\(\\s*['\"]([A-Za-z0-9_.]+)['\"]\\s*\\)", 'g');
          for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            for (const m of line.matchAll(re)) {
              // Skip if this looks like a DOM method call (createElement, getElementById, etc.)
              if (line.includes('document.') || line.includes('createElement') || line.includes('getElementById') || line.includes('querySelector')) {
                continue;
              }
              // Skip single character keys (likely HTML elements)
              if (m[1].length === 1) {
                continue;
              }
              // Skip if this appears to be an object property or parameter (preceded by : or =)
...
// Reset state on each load to avoid stale cache issues
