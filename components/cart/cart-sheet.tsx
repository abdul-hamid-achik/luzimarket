  ...
  return (
    <Sheet open={state.isOpen} onOpenChange={toggleCart}>
      <SheetContent className="w-full sm:max-w-md">
        <SheetHeader>
          <SheetTitle className="font-univers">Carrito ({getTotalItems()})</SheetTitle>
        </SheetHeader>
                ...
                {state.items.map((item) => (
                  <div key={item.id} className="flex gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      <p className="text-xs text-gray-600 font-univers">Por {item.vendorName}</p>
          ...
          {state.items.length > 0 && (
            <div className="border-t pt-6 pb-2 space-y-4">
              <div className="flex justify-between items-center">
                <span className="font-univers text-lg">Total</span>
